#!/usr/bin/env python3
"""Build a release provenance ledger for the latest completed V2 phase."""
...
from sl_legal_rag.operations import build_release_provenance_ledger, load_release_provenance_manifest  # noqa: E402


DEFAULT_MANIFEST = PROJECT_ROOT / "rag" / "evals" / "phase12_release_provenance.json"
DEFAULT_OUTPUT = PROJECT_ROOT / "logs" / "release-artifacts" / "phase12-release-provenance-ledger.json"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--git-json", help="Use saved git metadata JSON instead of calling git.")
...
def main(argv: list[str]) -> int:
    ...
    manifest = load_release_provenance_manifest(resolve_path(args.manifest))
    ...
    return 0 if args.allow_failures or ledger["status"] == "verified" else 1
