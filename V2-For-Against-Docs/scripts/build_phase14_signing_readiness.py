#!/usr/bin/env python3
"""Build the release signing readiness report for the latest completed V2 phase."""
...
DEFAULT_MANIFEST = PROJECT_ROOT / "rag" / "evals" / "phase14_release_signing_readiness.json"
DEFAULT_OUTPUT = PROJECT_ROOT / "logs" / "release-artifacts" / "phase14-signing-readiness.json"
...
def main(argv: list[str]) -> int:
    ...
    manifest = load_release_signing_readiness_manifest(resolve_path(args.manifest))
    ...
    return 0 if args.allow_blocked or report["status"] == "ready_for_signing_review" else 1
