...
case "${MODE}" in
  ...
  hosted-evidence-capture-runner)
    ...
    ;;
  hosted-capture-acceptance)
    COMMAND="PYTHONPATH=rag uv run --with pydantic python scripts/build_phase37_hosted_capture_acceptance.py --output logs/readiness/phase37-hosted-capture-acceptance.json"
    ;;
  hosted-capture-execution)
    COMMAND="PYTHONPATH=rag uv run --with pydantic python scripts/run_phase38_hosted_capture_execution.py --output logs/readiness/phase38-hosted-capture-execution.json"
    ;;
  ...
  production-cutover-readiness)
    COMMAND="PYTHONPATH=rag uv run --with pydantic python scripts/build_phase43_production_cutover_readiness.py --output logs/readiness/phase43-production-cutover-readiness.json"
    ;;
  ...
  production-cutover-execution-plan)
    COMMAND="PYTHONPATH=rag uv run --with pydantic python scripts/build_phase45_production_cutover_execution_plan.py --output logs/readiness/phase45-production-cutover-execution-plan.json"
    ;;
  post-cutover-monitoring-handover)
    COMMAND="PYTHONPATH=rag uv run --with pydantic python scripts/build_phase46_post_cutover_monitoring_handover.py --output logs/readiness/phase46-post-cutover-monitoring-handover.json"
    ;;
  load)
